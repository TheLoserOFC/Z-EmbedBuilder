...
<div class="embed markup">
    <div class="embedGrid">
        <div class="embedAuthor embedMargin" style="display: flex;">
            <img class="embedAuthorIcon embedAuthorLink"
                src="https://cdn.discordapp.com/embed/avatars/0.png">
            <a class="embedAuthorNameLink embedLink embedAuthorName"
                href="https://discord.com" target="_blank">Author name</a>
        </div>
        <div class="embedTitle embedMargin" style="display: unset;"><a
                class="anchor" target="_blank"
                href="https://discord.com">Title</a></div>
        <div class="embedDescription embedMargin"
            style="display: unset;">
            description</div>
        <div class="embedFields" style="display: grid;">
            <div class="embedField" style="grid-column: 1 / 13;">
                <div class="embedFieldName">Another field</div>
                <div class="embedFieldValue">
                    <div class="blockquote">
                        <div class="blockquoteDivider"></div>
                        <blockquote>Nope, didn't forget about this
                        </blockquote>
                    </div>
                </div>
            </div>
        ...
        <div class="imageWrapper clickable embedMedia embedImage"
            style="display: block;">
            <img class="img embedImageLink"
                onload="this.nextElementSibling?.style.removeProperty('display');"
                src="https://glitchii.github.io/embedbuilder/assets/media/banner.png">
        ...
        <div class="imageWrapper clickable embedThumbnail"
            style="display: block;">
            <img class="img embedThumbnailLink"
                onload="this.nextElementSibling?.style.removeProperty('display');"
                src="https://cdn.discordapp.com/embed/avatars/0.png">
            <div class="spinner-container" style="">
                <span class="spinner">
        ...
        <div class="embedFooter embedMargin" style="display: flex;">
           <span
...
function validateEmbedJson(embedJson) {
    ...
    for (const [key, type] of Object.entries(requiredFields)) {
        if (embedJson[key] && typeof embedJson[key] !== type) {
            errors.push(`Campo "${key}" deve ser do tipo ${type}.`);
        }

        if (key === 'footer' || key === 'author') {
            if (embedJson[key] && typeof embedJson[key] === 'object') {
                if (!embedJson[key].text || typeof embedJson[key].text !== 'string') {
                    errors.push(`Campo "${key}.text" é obrigatório e deve ser uma string.`);
                }

                if (embedJson[key].icon_url && typeof embedJson[key].icon_url !== 'string') {
                    errors.push(`Campo "${key}.icon_url" deve ser uma string contendo uma URL.`);
                }
            }
        }

        if (key === 'fields' && Array.isArray(embedJson[key])) {
            embedJson[key].forEach((field, index) => {
                if (!field.name || typeof field.name !== 'string') {
                    errors.push(`Campo "fields[${index}].name" é obrigatório e deve ser uma string.`);
                }
                if (!field.value || typeof field.value !== 'string') {
                    errors.push(`Campo "fields[${index}].value" é obrigatório e deve ser uma string.`);
                }
                if (field.inline && typeof field.inline !== 'boolean') {
                    errors.push(`Campo "fields[${index}].inline" deve ser um booleano.`);
                }
            });
        }
    }
...
const initializeGUI = (embedData) => {

    if (!embedData) {
        $('.embedPreview').replaceWith(initialEmbed);
        return
    }

    //const valid = validateEmbedJson(embedData)
...
function updateGUI(jsonData) {
    var embedGrid = $('.embedGrid');

    if (jsonData.color) {
        var borderColor = typeof jsonData.color === 'number' ? '#' + jsonData.color.toString(16).padStart(6, "0") : jsonData.color;
        embedGrid.closest('.embed').css('border-color', borderColor);
    } else {
        embedGrid.closest('.embed').css('border-color', '');
    }

    var titleContainer = $('.embedTitle');
    titleContainer.empty();
    if (jsonData.title) {
        var titleElement = jsonData.url ? $('<a>').addClass('anchor') : $('<span>');
        titleElement.html(markup(jsonData.title, { replaceEmojis: true, inlineBlock: false, inEmbed: true }));
        if (jsonData.url) {
            titleElement.attr('href', jsonData.url);
        }
        titleContainer.append(titleElement).css('display', 'unset');
    } else {
        titleContainer.css('display', 'none');
    }

    var descriptionElement = $('.embedDescription');
    if (jsonData.description) {
        descriptionElement.html(markup(jsonData.description, { replaceEmojis: true, inlineBlock: false, inEmbed: true })).css('display', 'unset');
    } else {
        descriptionElement.css('display', 'none');
    }

    if (jsonData.author && jsonData.author.name) {
        var authorContainer = $('.embedAuthor');
        var authorNameElement;

        authorContainer.empty();

        if (jsonData.author.url) {
            authorNameElement = $('<a>').addClass('embedAuthorNameLink embedLink embedAuthorName').attr('href', jsonData.author.url);
        } else {
            authorNameElement = $('<span>').addClass('embedAuthorName');
        }

        authorNameElement.text(jsonData.author.name);
        authorContainer.append(authorNameElement);

        if (jsonData.author.icon_url) {
            var authorIcon = $('<img>').addClass('embedAuthorIcon embedAuthorLink').attr('src', jsonData.author.icon_url);
            authorContainer.prepend(authorIcon);
        }

        $('.embedAuthor').css('display', 'flex');
    } else {
        $('.embedAuthor').css('display', 'none');
    }

    var imageContainer = $('.imageWrapper.embedImage');
    var imageElement = $('.embedImageLink');
    if (jsonData.image && jsonData.image.url) {
        imageElement.attr('src', jsonData.image.url);
        imageContainer.css('display', 'block');
    } else {
        imageContainer.css('display', 'none');
    }

    var thumbnailContainer = $('.imageWrapper.embedThumbnail');
    var thumbnailElement = $('.embedThumbnailLink');
    if (jsonData.thumbnail && jsonData.thumbnail.url) {
        thumbnailElement.attr('src', jsonData.thumbnail.url);
        thumbnailContainer.css('display', 'block');
    } else {
        thumbnailContainer.css('display', 'none');
    }

    if (jsonData.footer && jsonData.footer.text) {
        var footerContainer = $('.embedFooter');

        footerContainer.empty();

        var footerTextElement = $('<span>').addClass('embedFooterText').text(jsonData.footer.text);

        footerContainer.append(footerTextElement);


        if (jsonData.footer.icon_url) {
            var footerIconElement = $('<img>').addClass('embedFooterIcon embedFooterLink').attr('src', jsonData.footer.icon_url);
            footerContainer.prepend(footerIconElement);
        }

        footerContainer.css('display', 'flex');
    } else {
        $('.embedFooter').css('display', 'none');
    }

    var fieldsContainer = $('.embedFields');
    fieldsContainer.empty();
    if (jsonData.fields && jsonData.fields.length > 0) {
        jsonData.fields.forEach(function (field) {
            var fieldDiv = $('<div>').addClass('embedField').css('grid-column', field.inline ? 'span 6' : '1 / 13');
            var fieldNameDiv = $('<div>').addClass('embedFieldName').html(markup(field.name, { replaceEmojis: true, inlineBlock: false, inEmbed: true }));
            var fieldValueDiv = $('<div>').addClass('embedFieldValue').html(markup(field.value, { replaceEmojis: true, inlineBlock: false, inEmbed: true }));

            fieldDiv.append(fieldNameDiv).append(fieldValueDiv);
            fieldsContainer.append(fieldDiv);
        });
        fieldsContainer.css('display', 'grid');
    } else {
